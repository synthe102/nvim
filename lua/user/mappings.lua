...
wk.register({
  ["<leader>"] = {
    ...
    l = {
      ...
      h = { function() vim.lsp.buf.hover() end, "Toggle LSP hover" },
    },
    h = {
      name = "Harpoon",
    ...
    c = { function() require("Comment.api").toggle.linewise.current() end, "Comment current line" },
    d = {
      name = "DAP",
...
vim.api.nvim_create_autocmd("BufWritePre", {callback= function() vim.diagnostic.setloclist({open=false}) end})
vim.api.nvim_create_autocmd("BufWritePre", {
  callback = function() require('lsp-zero.server').format_cmd() end,
})
